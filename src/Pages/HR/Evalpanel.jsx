...
import { GitHubAnalytics, LeetCodeAnalytics, LinkedInAnalytics } from "./AnalyticsComponents.jsx";
...
const Tip = ({ active, payload, label }) => {
    ...
    return (
        <div style={{ background: "rgba(0, 0, 0, 0.64)", border: "1px solid #d8d8d8", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#ccc", backdropFilter: "blur(12px)" }}>
            {label && <p style={{ fontWeight: 600, color: "#fff", marginBottom: 4 }}>{label}</p>}
...
export default function EvalPanel({ evalData, evalSummary, candidate }) {
    ...
    const lcDiff = [
        { name: "Easy", value: lc.easy || 0, color: "#aada00" },
        { name: "Medium", value: lc.medium || 0, color: "#f59e0b" },
        { name: "Hard", value: lc.hard || 0, color: "#ff4400" },
    ];
        ...
        <div className="ep-root">
            ...
            {lc.total ? (
                <div className="ep-row ep-row-leetcode">
                    {/* Donut */}
                    <GlassCard className="gc-lc-donut" >
                        <div className="ep-bg-texture"  />
                        ...
                        <div style={{ position: "relative", display: "flex", justifyContent: "center", margin: "8px 0" }}>
                            <PieChart width={260} height={260}>
                                <Pie data={lcDiff.filter(d => d.value > 0)} cx={128} cy={128}
                                    ...
                                    dataKey="value" strokeWidth={3} stroke="rgba(255, 0, 0, 0)">
                                    {lcDiff.map((d, i) => <Cell key={i} fill={d.color}  />)}
                                </Pie>
                                <Tooltip content={<Tip />} />
                            </PieChart>
                            <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", textAlign: "center", pointerEvents: "none" }}>
                                <p style={{ fontSize: 46, fontWeight: 600, color: "#ff7700", lineHeight: 1 }}>{lc.total}</p>
                        ...
                        <div className="gc-lc-legend">
                            {lcDiff.map(d => (
                                <div key={d.name} className="gc-lc-legend-item">
                                    ...
                                    <span style={{ color: "#5f5f5f", fontSize: 13 }}>{d.name}</span>
                    ...
                    <GlassCard className="gc-lc-bar" style={{ background: "linear-gradient(135deg, #080808 0%, #0f0f0f 100%)" }}>
                        ...
                        <div style={{ flex: 1, minHeight: 0 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={lcDiff} margin={{ top: 8, right: 4, left: -12, bottom: 0 }}>
                                    ...
                                    <XAxis dataKey="name" tick={{ fill: "#3e3e3e", fontSize: 14, fontWeight: 500 }} axisLine={false} tickLine={false} />
                        ...
                        <div className="gc-lc-stats">
                            ...
                            <div className="gc-lc-stat" style={{ borderColor: "#c8ff0033" }}>
                                <p style={{ fontSize: 24, fontWeight: 900, color: "#28ab00" }}>{lc.active_days || 0}</p>
