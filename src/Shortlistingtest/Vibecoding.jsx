...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function VibeCoding({ token, test, onComplete }) {
    const [files, setFiles] = useState(() => makeDefaultFiles(test.starter_code, test.languages));
    const [activeId, setActiveId] = useState(1);
    const [nextId, setNextId] = useState(2);
    const [output, setOutput] = useState("");
    const [running, setRunning] = useState(false);
    const [outputTab, setOutputTab] = useState("question"); // ← opens on Question tab
    ...
    const handleRun = async () => {
        ...
        setOutputTab("output");
        try {
            const res = await fetch(`${API}/test/${token}/run`, {
                method: "POST",
    ...
    const sendChat = async () => {
        ...
        setChatInput("");
        try {
            const res = await fetch(`${API}/test/${token}/chat`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    message: chatInput,
    ...
    const applyCode = async (snippet) => {
        ...
        try {
            const res = await fetch(`${API}/test/${token}/edit`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    ...
                    <div className="vc-terminal" style={{ height: `${100 - editorH - 2}%` }}>
                        <div className="vc-terminal-bar">
                            <div className="vc-term-tabs">
                                {/* ── Question tab (first) ── */}
                                <button
                                    className={`vc-tab${outputTab === "question" ? " active" : ""}`}
                                    onClick={() => setOutputTab("question")}
                                >
                                    Question
                                </button>
                                <button
                                    className={`vc-tab${outputTab === "output" ? " active" : ""}`}
                                ...
                                {hasPreview && (
                                    <button
                                        className={`vc-tab${outputTab === "preview" ? " active" : ""}`}
                        ...
                        {/* ── Question panel ── */}
                        {outputTab === "question" && (
                            <div className="vc-question-panel">
                                {/* Problem statement */}
                                <div className="vc-question-section">
                                    <p className="vc-question-label">Problem</p>
                                    <p className="vc-question-text">
                                        {test.problem_statement || "No problem statement available."}
                                    </p>
                                </div>
                                ...
                                {test.constraints && (
                                    <div className="vc-question-section">
                                        <p className="vc-question-label">Constraints</p>
                                        <p className="vc-question-text">{test.constraints}</p>
                                    </div>
                                )}
                                ...
                                {test.examples && test.examples.length > 0 && (
                                    <div className="vc-question-section">
                                        <p className="vc-question-label">Examples</p>
                                        <div className="vc-examples-list">
                                            {test.examples.map((ex, i) => (
                                                <div key={i} className="vc-example-block">
                                                    <div className="vc-example-num">Example {i + 1}</div>
                                                    <div className="vc-example-row">
                                                        <span className="vc-example-key">Input</span>
                                                        <code className="vc-example-val">{ex.input}</code>
                                                    </div>
                                                    <div className="vc-example-row">
                                                        <span className="vc-example-key">Output</span>
                                                        <code className="vc-example-val">{ex.output}</code>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
